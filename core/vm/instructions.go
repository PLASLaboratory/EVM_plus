...
<<<<<<< HEAD
/*ForFallback is structure for flag that releated with fallback*/
/*Sooyeon LEE 2020.02.25- to use this keywords at other go file*/
type ForFallback struct {
	IsNonFallBackEnforced bool
	StartingNonFallback   bool
}

/*NewForFallback is structure for flag that releated with fallback*/
func NewForFallback() *ForFallback {
	return &ForFallback{}
}
...
//2020.02.25 addd to new geth file
...
func opEndFallback(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	fmt.Printf("%s", "end non fallback\n")
	return nil, nil
}

/*SooYeon LEE 20191201 checking init*/
func opForInitStart(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	//fallbackFlag.StartingNonFallback = false
	fmt.Printf("%s", "for init \n")
	return nil, nil
}
